...
class TestNormalizeKeywords(unittest.TestCase):
    def test_comma_separated_string(self):
        self.assertEqual(main._normalize_keywords("a, b, c"), ["a", "b", "c"])

    def test_semicolon_separated_string(self):
        self.assertEqual(main._normalize_keywords("a; b; c"), ["a", "b", "c"])

    def test_single_value(self):
        self.assertEqual(main._normalize_keywords("tag"), ["tag"])

    def test_list_input(self):
        self.assertEqual(main._normalize_keywords(["a", "b"]), ["a", "b"])

    def test_none_returns_empty(self):
        self.assertEqual(main._normalize_keywords(None), [])

    def test_empty_string_returns_empty(self):
        self.assertEqual(main._normalize_keywords(""), [])
...
class TestDedupePreserveOrder(unittest.TestCase):
    def test_removes_case_insensitive_duplicates(self):
        self.assertEqual(main._dedupe_preserve_order(["tag", "Tag", "TAG"]), ["tag"])

    def test_preserves_first_occurrence(self):
        self.assertEqual(main._dedupe_preserve_order(["Tag", "tag"]), ["Tag"])

    def test_preserves_order(self):
        self.assertEqual(main._dedupe_preserve_order(["b", "a", "c"]), ["b", "a", "c"])

    def test_empty_list(self):
        self.assertEqual(main._dedupe_preserve_order([]), [])


class TestMergeKeywords(unittest.TestCase):
    def test_set_replaces_existing(self):
        self.assertEqual(main.merge_keywords(["old"], ["new"], "set"), ["new"])

    def test_set_dedupes_incoming(self):
        self.assertEqual(main.merge_keywords([], ["a", "A"], "set"), ["a"])

    def test_replace_is_alias_for_set(self):
        self.assertEqual(main.merge_keywords(["old"], ["new"], "replace"), ["new"])

    def test_append_combines(self):
        self.assertEqual(main.merge_keywords(["a"], ["b"], "append"), ["a", "b"])

    def test_append_dedupes(self):
        self.assertEqual(main.merge_keywords(["a"], ["a", "b"], "append"), ["a", "b"])

    def test_append_case_insensitive_dedupe(self):
        self.assertEqual(main.merge_keywords(["Tag"], ["tag", "new"], "append"), ["Tag", "new"])

    def test_set_empty_clears(self):
        self.assertEqual(main.merge_keywords(["old"], [], "set"), [])

    def test_unknown_mode_raises(self):
        with self.assertRaises(ValueError):
            main.merge_keywords([], [], "unknown")
...
class TestGetKeywords(unittest.TestCase):
    ...
    def test_reads_from_metadata_dict(self):
        ...
        self.assertEqual(main.get_keywords(item), ["a", "b"])
    ...
    def test_falls_back_to_explicit_key(self):
        ...
        self.assertEqual(main.get_keywords(item), ["a"])
    ...
    def test_falls_back_to_clip_property(self):
        ...
        self.assertEqual(main.get_keywords(item), ["x", "y"])

    def test_returns_empty_when_nothing(self):
        item = MagicMock()
        item.GetMetadata.side_effect = lambda key=None: {} if key is None else None
        item.GetClipProperty.return_value = ""
        self.assertEqual(main.get_keywords(item), [])
